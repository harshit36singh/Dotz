...
class HomeViewModel extends ChangeNotifier {
  ...
  Future<void> checkLive() async {
    try {
      final v =
          await _channel.invokeMethod<bool>('isLiveWallpaperActive') ?? false;
      _live = v;
      notifyListeners();
    } catch (_) {}
  }
  ...
  int get goalTotal =>
      _goalDate == null
          ? 365
          : (_goalDate!.difference(DateTime.now()).inDays * 2).clamp(1, 999999);
  ...
  int get daysLived =>
      _birthDate == null
          ? 0
          : DateTime.now().difference(_birthDate!).inDays;
  ...
  String get heroBigNum {
    ...
    return switch (_mode) {
      ...
      CalendarMode.goal => goalDaysLeft > 99
          ? '$goalDaysLeft'
          : goalDaysLeft.toString().padLeft(2, '0'),
  ...
  String get heroStatB {
    ...
    return switch (_mode) {
      CalendarMode.year =>
          '— ${(yp * 100).toStringAsFixed(0)}% of the year',
  ...
  /// Returns true on success, false on failure (caller shows sheet either way).
